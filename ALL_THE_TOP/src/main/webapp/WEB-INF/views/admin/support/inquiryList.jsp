							...
							<tbody>
								<c:forEach var="supportDTO" items="${inquiryList}">
									<tr>
										...
										<c:set var="maxLength" value="20" /> <!-- 표시할 최대 길이를 설정 -->
								            <script>
								                var suggestion = "${supportDTO.suggestion}";
								                var subSugestion;
								                var maxLength = ${maxLength};
								                if (suggestion.length > maxLength) {
								                	subSugestion = suggestion.substring(0, maxLength) + "...";
								                    console.log(subSugestion);
								                    var tdElement = document.createElement("td");
								                    tdElement.textContent = subSugestion;
								                }
								                else{
								                	var tdElement = document.createElement("td");
